...
@section('page-script')
    ...
    <script type="module">
        let selections = [];
        let routes = {'delete-diagnoses': '{{ route('guides-removeDiagnoses') }}'};
        ...
        function initTable() {
            $table.bootstrapTable({
                columns: [
                    {field: 'name', title: 'Диагноз', align: 'center', sortable: !0},
    ...
    <div class="modal fade" id="updateGuide">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <form class="ajax-post-form" method="POST" action="{{ route('guides-editDiagnoses') }}">
                    <div class="modal-body">
                        <input type="hidden" name="id" id="diagnoses_id">
                        <div class="mb-2">
                            <label class="form-label" for="name">Диагноз</label>
                            <input name="name" id="name" type="text" class="form-control form-control-sm" placeholder="Диагноз" required>
                        </div>
                    </div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header border-bottom">
                    <h4 class="card-title m-0">Справочник - диагнозы</h4>
                </div>
                <div class="card-body">
                    <div id="toolbar">
                        <button type="button" id="command" class="btn btn-sm btn-outline-success width-150">
                            <i class="fas fa-plus me-2"></i> Добавить диагноз
                        </button>
                    </div>
                    <table id="bootstrap_tbl"
                           ...
                           data-url="{{ route('guides-getDiagnoses') }}">
                    </table>
                </div>
            </div>
        </div>
